...
if (!defined('SITE_PIN_DEBUG'))
  define('SITE_PIN_DEBUG', true);

add_action('template_redirect', 'site_pin_template_redirect');
function site_pin_template_redirect() {
  if (SITE_PIN_DEBUG) do_action('log', 'Site PIN: init');
  $enabled = (boolean) get_option('site_pin_enable', true);
  $pin = (string) get_option('site_pin_code', "0000");
...
function site_pin_readout() {
  $enabled = (boolean) get_option('site_pin_enable', true);
  $pin = (string) get_option('site_pin_code', "0000");
  $message = (string) get_option('site_pin_message', '');
...
function site_pin_settings() {
  ...
  switch ($action) {
    case "save":
      ...
      update_option('site_pin_enable', $enable);

      $pin = $_REQUEST['site_pin_code'];
      update_option('site_pin_code', $pin);

      $message = $_REQUEST['site_pin_message'];
      update_option('site_pin_message', $message);
      break;
  }
